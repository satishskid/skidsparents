...
const CREATE_PILOT_INVITATIONS_TABLE = `
...
const CREATE_PILOT_GROUPS_TABLE = `
...
const CREATE_PILOT_ENGAGEMENT_LOG_TABLE = `
...
const CREATE_PED_APPLICATIONS_TABLE = `
...
async function handleInit(request: Request, locals: App.Locals) {
  ...
  try {
    ...
    await db.prepare(CREATE_PILOT_INVITATIONS_TABLE).run()
    await db.prepare(CREATE_PILOT_GROUPS_TABLE).run()
    ...
    return new Response(
      JSON.stringify({ success: true, message: 'All 43 tables ready (18 core + 9 life record + 4 daily experience + 8 growth & intervention + 4 pilot program) + columns migrated' }),
