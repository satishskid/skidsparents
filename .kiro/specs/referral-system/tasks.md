...
- [ ] 1. Create D1 migration for referral system
...
- [ ] 2. Update Drizzle schema
  - [ ] 2.1 Add `referralCode` and `isChampion` columns to the `parents` table in `src/lib/db/schema.ts`
    - `referralCode: text('referral_code').unique()`
    - `isChampion: integer('is_champion', { mode: 'boolean' }).default(false)`
    - _Requirements: 1.1, 6.1_
  - [ ] 2.2 Add `referrals` table export to `src/lib/db/schema.ts`
...
- [ ] 3. Implement referral code generation
  - [ ] 3.1 Create `src/lib/referral/generateCode.ts` exporting `generateReferralCode(firebaseUid: string): Promise<string>`
...
- [ ] 4. Implement referral link construction
  - [ ] 4.1 Create `src/lib/referral/buildLink.ts` exporting `buildReferralLink(code: string, medium: 'whatsapp' | 'copy'): string`
...
- [ ] 5. Create referral landing page
...
- [ ] 6. Update `LoginForm.tsx` to forward referral code
...
- [ ] 7. Update session API to generate code and trigger attribution
...
- [ ] 8. Create `POST /api/referrals/attribute` route
...
- [ ] 9. Create `GET /api/referrals/stats` route
...
- [ ] 11. Wire `ReferralBanner.tsx` to real API
...
- [ ] 12. Update `UserProfile.tsx` to show SKIDS Champion badge
...
- [ ] 13. Write unit tests for `generateCode` and `buildLink`
