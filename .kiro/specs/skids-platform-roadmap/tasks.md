...
- [ ] 1. Phase 1 — DB schema additions and migrations
  - [ ] 1.1 Add new columns to `providers` table in `src/lib/db/schema.ts`
    - Add `firebaseUid` (text, unique), `status` (enum: pending_review | active | suspended), `feeStructureJson` (text), `medicalRegNumber` (text)
    - _Requirements: 3.1, 3.7_

  - [ ] 1.2 Add new columns to `serviceOrders` table in `src/lib/db/schema.ts`
  ...
  - [ ] 1.3 Add `providerSlots` table to `src/lib/db/schema.ts`
  ...
  - [ ] 1.4 Add `providerCredentials` table to `src/lib/db/schema.ts`
    - Fields: id, providerId (FK), fileUrl, fileType (enum: pdf | image), docType, uploadedAt
    - _Requirements: 3.3_

  - [ ] 1.5 Add `sessionNotes` table to `src/lib/db/schema.ts`
    - Fields: id, orderId (FK), providerId (FK), noteText, createdAt
    - _Requirements: 4.5, 4.6_

  - [ ] 1.6 Add `prescriptions` table to `src/lib/db/schema.ts`
  ...
  - [ ] 1.7 Add `auditLog` table to `src/lib/db/schema.ts`
  ...
  - [ ] 1.8 Write migration SQL file `migrations/0003_platform_roadmap.sql`
...
- [ ] 2. Phase 2 — Razorpay payment integration

  - [ ] 2.1 Implement pure utility functions in `src/lib/payments.ts`
  ...
  - [ ] 2.5 Implement `POST /api/payments/create-order` in `src/pages/api/payments/create-order.ts`
  ...
  - [ ] 2.6 Implement `POST /api/payments/verify` in `src/pages/api/payments/verify.ts`
  ...
  - [ ] 2.7 Implement `POST /api/payments/webhook` in `src/pages/api/payments/webhook.ts`
  ...
  - [ ] 2.8 Implement `POST /api/payments/refund` in `src/pages/api/payments/refund.ts`
    - ADMIN_KEY auth; call Razorpay Refunds API for full `amount_cents`; write audit_log entry
    - _Requirements: 8.5, 8.6_

  - [ ] 2.9 Checkpoint — Ensure all tests pass, ask the user if questions arise.

- [ ] 3. Phase 3 — Booking flow (parent side)

  - [ ] 3.1 Implement `GET /api/bookings/providers` in `src/pages/api/bookings/providers.ts`
    - Authenticate parent; accept `serviceId` query param; return verified providers with available slots
    - _Requirements: 7.3_

  - [ ] 3.2 Implement `GET /api/bookings/slots` in `src/pages/api/bookings/slots.ts`
  ...
  - [ ] 3.4 Implement `GET /api/bookings/orders` in `src/pages/api/bookings/orders.ts`
    - Authenticate parent; return all service_orders for that parent with service and provider details joined
    - _Requirements: 7.6_

  - [ ] 3.5 Implement `GET /api/session/:orderId/token` in `src/pages/api/session/[orderId]/token.ts`
  ...
  - [ ] 3.5 Build `BookingFlow` React component in `src/components/services/BookingFlow.tsx`
  ...
  - [ ] 3.8 Checkpoint — Ensure all tests pass, ask the user if questions arise.

- [ ] 4. Phase 4 — Teleconsultation session room (highest priority clinical surface)

  - [ ] 4.1 Implement provider auth helper `src/pages/api/provider/_auth.ts`
  ...
  - [ ] 4.2 Implement `POST /api/provider/orders/:id/session/start` in `src/pages/api/provider/orders/[id]/session/start.ts`
  ...
  - [ ] 4.3 Implement `GET /api/provider/orders/:id/ai-brief` in `src/pages/api/provider/orders/[id]/ai-brief.ts`
  ...
  - [ ] 4.4 Implement `POST /api/provider/orders/:id/ai-assist` in `src/pages/api/provider/orders/[id]/ai-assist.ts`
  ...
  - [ ] 4.5 Implement `POST /api/provider/orders/:id/prescription` in `src/pages/api/provider/orders/[id]/prescription.ts`
  ...
  - [ ] 4.7 Implement `GET /api/children/:childId/prescriptions` in `src/pages/api/children/[childId]/prescriptions.ts`
  ...
  - [ ] 4.8 Build `SessionRoom` React component in `src/components/teleconsult/SessionRoom.tsx`
  ...
  - [ ] 4.9 Create Astro page `src/pages/provider/orders/[id]/session.astro`
    - SSR; authenticate provider (redirect to `/provider/login` if not); mount `<SessionRoom client:load />`
    - _Requirements: 4.1_

  - [ ] 4.10 Update `src/components/phr/RecordsTimeline.tsx` to display prescriptions
  ...
  - [ ] 4.11 Checkpoint — Ensure all tests pass, ask the user if questions arise.

- [ ] 5. Phase 5 — Provider portal

  - [ ] 5.1 Implement `POST /api/provider/signup` in `src/pages/api/provider/signup.ts`
  ...
  - [ ] 5.2 Implement `GET /api/provider/me` and provider order/earnings routes
  ...
  - [ ] 5.3 Implement PHR access gate `GET /api/provider/phr/:childId` in `src/pages/api/provider/phr/[childId].ts`
  ...
  - [ ] 5.5 Implement slot management routes in `src/pages/api/provider/slots/`
  ...
  - [ ] 5.6 Implement `POST /api/provider/credentials` in `src/pages/api/provider/credentials.ts`
  ...
  - [ ] 5.7 Build `ProviderApp` React component tree in `src/components/provider/`
  ...
  - [ ] 5.8 Create Astro pages for provider portal under `src/pages/provider/`
  ...
  - [ ] 5.9 Checkpoint — Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Phase 6 — Admin extensions (D2C Manager layer)

  - [ ] 6.1 Implement provider management admin routes in `src/pages/api/admin/`
  ...
  - [ ] 6.2 Implement order management admin routes in `src/pages/api/admin/`
  ...
  - [ ] 6.3 Implement revenue and supply monitoring routes
  ...
  - [ ] 6.5 Implement `GET /api/admin/audit-log` in `src/pages/api/admin/audit-log.ts`
  ...
  - [ ] 6.7 Build admin React components in `src/components/admin/`
  ...
  - [ ] 6.8 Create admin Astro pages extending `src/pages/admin/`
  ...
  - [ ] 6.9 Implement WhatsApp retry worker in `cron-worker/index.ts`
  ...
  - [ ] 6.11 Checkpoint — Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Phase 7 — Marketing site (`skids.clinic`)

  - [ ] 7.1 Scaffold `marketing/` Astro project in this monorepo
  ...
  - [ ] 7.2 Build marketing site pages in `marketing/src/pages/`
  ...
  - [ ] 7.3 Build `LeadCaptureForm` React component in `marketing/src/components/LeadCaptureForm.tsx`
  ...
  - [ ] 7.4 Implement UTM preservation across all marketing site CTAs
  ...
  - [ ] 7.5 Verify `POST /api/lead` on parent app accepts cross-origin requests from `skids.clinic`
  ...
  - [ ] 7.6 Final checkpoint — Ensure all tests pass, ask the user if questions arise.
